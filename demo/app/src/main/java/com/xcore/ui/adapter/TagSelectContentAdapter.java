...
public class TagSelectContentAdapter extends BaseRecyclerAdapter<TagBean,TagSelectContentAdapter.ViewHolder>{
    private ITagOnClick tagOnClick;

    public TagSelectContentAdapter(Context mContext, ITagOnClick iTagOnClick) {
        super(mContext);
    ...
    private TextView getText(final Tag tag,final int position){
        ...
        if(tag.isChecked()){//选中了
            textView.setBackgroundResource(R.drawable.tag_select_bg);
        }else {
            textView.setTextColor(mContext.getResources().getColor(R.color.black_1A));
