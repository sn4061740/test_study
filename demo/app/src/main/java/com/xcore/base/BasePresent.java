...
public class BasePresent<T> {
    ...
    public LoadingDailog getDialog() {
        return dialog;
    }
    ...
    public void joinView(String className,String paramsStr){
        ...
        ApiFactory.getInstance().<LikeBean>joinView(className,paramsStr, new TCallback<LikeBean>() {
            @Override
    ...
    public void outView(String className,String paramsSt){
        if(exitId==null||exitId.length()<=0||exitId.equals("")){
            return;
        }
        try {
            ApiFactory.getInstance().<String>outView(exitId, paramsSt, new TCallback<String>() {
                @Override
