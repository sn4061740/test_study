...
public class MySQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    private MySQLiteOpenHelper(Context context, String name) {
        this(context, name, null, 15);//传入Context和数据库的名称，调用上面那个构造器
    }
    //将自定义的数据库创建类单例。
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        String commentSql = "create table if not exists "
                + TableConfig.COMMENT.TABLE_COMMENT + "("
                + TableConfig.COMMENT.COMMENT_ID + " integer not null primary key autoincrement,"
                + TableConfig.COMMENT.COMMENT_SHORT_ID+ " verchar(255),"
                + TableConfig.COMMENT.COMMENT_DELETE + " verchar(10),"
                + TableConfig.COMMENT.COMMENT_TIME + " verchar(255)"
                + ")";
        sqLiteDatabase.execSQL(commentSql);

        String newCacheSql = "create table if not exists "
                + TableConfig.NEW_CACHE.TABLE_NEW_CACHE + "("
                + TableConfig.NEW_CACHE.ID + " integer not null primary key autoincrement,"
                + TableConfig.NEW_CACHE.NEW_CACHE_ID + " verchar(255),"
                + TableConfig.NEW_CACHE.NEW_CACHE_TITLE + " verchar(255),"
                + TableConfig.NEW_CACHE.NEW_CACHE_CONVER + " verchar(255),"
                + TableConfig.NEW_CACHE.NEW_CACHE_URL + " verchar(255),"
                + TableConfig.NEW_CACHE.NEW_CACHE_PERCENT + " verchar(50),"
                + TableConfig.NEW_CACHE.NEW_CACHE_KEY+ " verchar(255),"
                + TableConfig.NEW_CACHE.NEW_CACHE_FILESIZE+ " verchar(255),"
                + TableConfig.NEW_CACHE.NEW_SHORT_ID+ " verchar(255)"
                + ")";
        sqLiteDatabase.execSQL(newCacheSql);
        Log.e("TAG", "创建表");
    }
    @Override
...
//
        String NEW_CACHE_TABLE = "DROP TABLE IF EXISTS "+TableConfig.NEW_CACHE.TABLE_NEW_CACHE;
        sqLiteDatabase.execSQL(NEW_CACHE_TABLE);
        Log.e("TAG","删除表 。");
