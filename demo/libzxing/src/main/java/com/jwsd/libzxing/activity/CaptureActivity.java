...
public class CaptureActivity extends BaseCommonPopupActivity implements
...
//                    CaptureActivity.this.finish();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK && requestCode == CODE_GALLERY_REQUEST) {
            final String picPath = SelectAlbumUtils.getPicPath(this, data);
            if(showLoadingListener!=null){
                showLoadingListener.show();
            }
            ...
            new Thread(new Runnable() {
                ...
                if (result != null) {
                    ...
                    try {
                        beepManager.playBeepSoundAndVibrate();
                        scanResult = DecodeBitmap.parseReuslt(result.toString());
                        Thread.sleep(1000);
                    }catch (Exception ex){}
                    scanDeviceSuccess(scanResult, new Bundle());
                }else{
                    scanDeviceSuccess("", new Bundle());
                }
                }
            }).start();
